...
terminal summary, ticket-ready text, and a self-contained HTML report.
...
The repository ships a deliberately misconfigured AWS account. It is emulated
in-process by moto, so this needs no credentials, no account, and no network —
and everything on the auditor's side of the endpoint is real boto3, real
botocore signing and parsing, real paginators, real AWS error codes.

```bash
docker build -t account-access-auditor .
docker run --rm -v "$PWD/reports:/out" account-access-auditor
```
...
python -m lab.demo --output-dir reports
```

Either one scans the lab account and writes the report in all four formats.
`docker run --rm account-access-auditor verify` instead re-derives every number
in the table above — test suite, coverage, self-audit, policy-drift check, and
the scored benchmark — in one command.

![Terminal output of a scan of the emulated lab account](docs/images/terminal.svg)

### The HTML report

`--output html` writes one self-contained file. It is the same report as the
JSON beside it, rendered for a person rather than a parser.

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/images/report-dark.png">
  <img alt="The HTML report: 25 findings, a severity distribution bar, and the five-tier legend" src="docs/images/report-light.png">
</picture>

Each finding carries its evidence, its rationale, its remediation, and its CIS
control, and the severity chips filter the list:

![A finding in the HTML report, with its evidence, rationale and remediation](docs/images/report-findings.png)

Three properties hold, each with a test behind it in `tests/test_reporters.py`:

**It contains no JavaScript.** Not "no third-party JavaScript" — none. Filtering
is radio inputs and CSS `:has()`, and the file declares `default-src 'none'`, so
it renders fully with scripts disabled. A findings report gets archived and
reopened years later; it should not be a program.

**It loads nothing from the network.** No CDN, no web font, no pixel. The file
works offline and air-gapped, and cannot report which account was scanned.

**Every string from the audited account is escaped.** Bucket names, IAM user
names, and policy documents are attacker-influenced input, and `<img onerror=…>`
is a legal S3 bucket name. The test suite renders a report whose every field
carries markup and asserts, with an HTML parser rather than a regex, that the
document contains no script element, no event-handler attribute, and no
attribute that dereferences a URL.

A sample is committed at [`results/aws_audit_report.html`](results/aws_audit_report.html),
regenerated by `python -m lab.run_benchmark`.

### The published demo

`scripts/build_site.py` assembles a page that replays a scan — the transcript
streams into a terminal, the severity counts and the scoring panel fill in
behind it, and a selector switches between the two lab profiles so you can watch
the rules respond to a different account. `.github/workflows/pages.yml` builds
and publishes it on every push to `main` (enable Pages once under Settings →
Pages → Source: GitHub Actions).

The page is assembled from `results/`, never typed in: the terminal transcript is
captured from `auditor.reporters.terminal`, the same reporter a real run prints
through, and every count is read out of the JSON artifacts. `tests/test_site_build.py`
fails if a number on the page stops matching the artifact behind it.

It says plainly that the scan is a replay rather than a scan running in the
visitor's browser, because a static page cannot make AWS API calls and pretending
otherwise would be the one dishonest thing on it. Note the split in posture: the
landing page uses JavaScript for the animation, while the audit report it links
to still contains none — the report is evidence and gets archived, the landing
page is a shop window.
...
| Tests passing | **611 / 611** | `pytest` |
| Detection-code coverage (branch-inclusive) | **99.87%** | `results/coverage.json` |
| Whole-package coverage (branch-inclusive) | 99.64% | `results/coverage.json` |
...
See [the scan above](#run-it-without-an-aws-account). That image is generated by
`python scripts/render_terminal_svg.py`, which runs the auditor and renders what
it actually printed — so it is regenerated rather than transcribed.
...
└── reporters/        json, csv, tickets, html, terminal
```
...
**611 tests, 98 offline fixture cases, 99.87% branch coverage of the detection
...
```bash
docker build -t account-access-auditor .
docker run --rm -v "$PWD/results:/out" account-access-auditor verify
```

...
regenerated by `python -m lab.run_benchmark`. `.github/workflows/pages.yml`
publishes that same file to GitHub Pages on every push to `main` — generated by
the run rather than copied from the commit, so the published page is always the
...
**601 tests, 98 offline fixture cases, 99.87% branch coverage of the detection
